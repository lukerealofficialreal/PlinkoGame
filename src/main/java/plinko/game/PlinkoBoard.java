...
//    }
    ...
    //removes any solidified balls from play
    public void solidifyObjectsAroundLocation(int xLoc, int yLoc) {
        ...
        for(int[] location : locations) {
            ...
            if(location != target && tile.getObj() instanceof PlinkoBallObject) {
                solidifyObjectsAroundLocation(location[0], location[1]);
            }
            //If the location contains a ball but is not the target, soldify the objects around that location as well
            if(tile.isOccupied() && tile.getObj().getOwnerId() != PlinkoObject.SERVER_ID) {
    ...
    public void scoreBall(PlinkoBallObject ball) {
        ...
        //for each ball in the score pit, remove from playerObjects, remove a ball from the field, and increment score
        //This handles the case where multiple balls enter a score pit at the same time
        for(PlinkoTile tile : scorePit) {
            if(tile.getObj() instanceof PlinkoBallObject) {
                playerObjectTiles.remove(tile);
                ballsOnField--;
                incrementScore();
            }
